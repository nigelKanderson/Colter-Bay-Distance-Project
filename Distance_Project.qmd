...
# Color effect by guild
em_guild_color <- emmeans(guild_model, ~ color | guild, type = "response")
cat("=== Color effect within each guild ===\n")
pairs(em_guild_color, adjust = "holm")
...
# Fig A: color effect by guild
em_guild_color_df <- as.data.frame(em_guild_color) %>%
  rename(lower_CL = asymp.LCL, upper_CL = asymp.UCL)

fig_guild_color <- ggplot(em_guild_color_df,
  ...
  scale_color_manual(values = c(R = col_red, W = col_white),
                     ...
                     name    = "Light color") +
  labs(
    title    = "Light Color Effect by Foraging Guild",
    subtitle = "Marginal means ± 95% CI from guild × color model",
    x        = "Foraging guild",
    y        = "Predicted detections per night"
  ) +
  bat_theme +
...
ggsave("output/figures/fig_guild_color.png",
       fig_guild_color, width = 7, height = 5, dpi = 300, bg = "white")
