...
# ── 3. Color × Distance interaction test ──────────────────────────────────────
em_dist_trend <- emtrends(simple_model1, ~ color,
                           var = "dist_km", type = "response")

cat("\n=== Distance slope (km) by color ===\n")
em_dist_trend

cat("\n=== Red vs. White distance slope contrast ===\n")
pairs(em_dist_trend)
```

## Distance × Color Figure

```{r dist_color_fig, warning=FALSE, message=FALSE}
library(emmeans)

# Predicted detections across the observed distance gradient, by color
dist_range <- range(data_env$dist_km, na.rm = TRUE)
dist_grid  <- seq(dist_range[1], dist_range[2], length.out = 60)

em_dist_color <- emmeans(
  simple_model1,
  ~ color | dist_km,
...
pred_df <- as.data.frame(em_dist_color) %>%
  rename(lower_CL = asymp.LCL, upper_CL = asymp.UCL)

ggplot(pred_df, aes(x = dist_km, y = response, color = color, fill = color)) +
  ...
  labs(
    title    = "Predicted Bat Detections by Distance and Light Color",
