...
## Bat-Moth Predation Co-occurrence Test

```{r bat_moth_predation, warning=FALSE, message=FALSE}
source("R/18_bat_moth_predation.R")
pred_test <- run_bat_moth_predation_test(data_env, insect_total_env, bat_theme, col_navy, col_purple)

summary(pred_test$model)
pred_test$fig_partial
```

## Moth Distance Gradient Across the Season

```{r moth_season_distance, warning=FALSE, message=FALSE}
source("R/19_moth_season_distance.R")
season_test <- run_moth_season_distance_test(insect_total_env, bat_theme, col_navy, col_red, col_purple)

summary(season_test$model)
season_test$fig_season_distance
season_test$fig_season_distance_relative
```
