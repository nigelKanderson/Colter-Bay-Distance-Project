...
# Overall stream = focal species + all compound (multi-species) labels. Compounds
# are retained here for the OVERALL activity model and figures, but are dropped
# from the per-species and community models (which use `data_env_focal`, built
# after the trait join below). Non-focal single species stay excluded.
data_env <- data_env %>%
  filter(species %in% focal_spp | str_detect(species, "/"))
...
To see *which* species drive the red-vs-white pattern, fit a model with a species × colour × intensity interaction (so each species has its own response shape, unlike `species_glmm`, which is random-intercept only) and plot each species' predicted curve. This is the model-based analog of R/08's Fig 12.

```{r model_effect_byspecies, warning=FALSE, message=FALSE, fig.width=13, fig.height=6.5}
library(emmeans)

# species-specific colour x intensity response (fixed 3-way interaction),
# same zero-filled grid as species_glmm
species_glmm_int <- glmmTMB(
  detections ~ species * color * intensity +
...
fig_eff_byspecies <- ggplot(
  ...
  labs(title = "Per-species intensity × color effect (model-based)",
       subtitle = "Predicted detections/night ± 95% CI; species × color × intensity model; y-axes free",
...
Rather than forcing ambiguous compound calls onto a single species, group every detection — single *or* compound — into the acoustic guild the classifier is confident about: a **low-frequency** guild (Epfu / Lano / Laci) and a **Myotis** guild (the seven Myotis). A detection joins a guild only if *every* candidate species in its label belongs to that guild; cross-guild calls (\~0.7% of detections) are dropped. This keeps \~99% of detections and models activity at the level SonoBat can actually resolve — and it separates guilds that respond to distance in opposite directions (so a pooled total cancels them out).

```{r cluster_setup, warning=FALSE, message=FALSE}
lowfreq_spp <- c("Epfu", "Lano", "Laci")
myotis_spp  <- c("Mylu", "Myvo", "Myev", "Myci", "Myca", "Myyu", "Myth")

# a label joins a guild only if EVERY candidate species is in that guild
assign_cluster <- function(x) {
  vapply(stringr::str_split(x, "/"), function(toks) {
    if (all(toks %in% lowfreq_spp)) "LowFreq"
    else if (all(toks %in% myotis_spp)) "Myotis"
    else NA_character_
  }, character(1))
}

# per-guild detections per site-night (single + compound detections combined)
cl_det <- data_env %>%
...
cluster_night <- site_night %>%
  select(site, date, color, intensity, dist_km, mean_phase,
         pct_nonforest, brightness_dark, jd_c) %>%
  tidyr::crossing(cluster = c("LowFreq", "Myotis")) %>%
  left_join(cl_det, by = c("site", "date", "cluster")) %>%
  mutate(detections = tidyr::replace_na(detections, 0),
         site = factor(site), cluster = factor(cluster))

# guild x colour x intensity, plus a guild-specific distance slope
cluster_glmm <- glmmTMB(
  detections ~ cluster * color * intensity + cluster * dist_km +
...
```{r cluster_figs, warning=FALSE, message=FALSE, fig.width=11, fig.height=5}
library(emmeans)
...
dgrid <- expand.grid(
  ...
  cluster   = factor(c("LowFreq", "Myotis"), levels = levels(cluster_night$cluster)),
...
fig_cluster_dist <- ggplot(dgrid, aes(dist_km, fit, color = cluster, fill = cluster)) +
  ...
  scale_color_manual(values = c(LowFreq = col_navy, Myotis = col_red), name = NULL) +
  scale_fill_manual(values = c(LowFreq = col_navy, Myotis = col_red), guide = "none") +
  labs(title = "Guild response to distance",
       subtitle = "Predicted detections/night ± 95% CI (at 50% red, other covariates at means)",
       x = "Distance from Colter Bay parking lot (km)",
       y = "Predicted detections / night") +
  bat_theme
